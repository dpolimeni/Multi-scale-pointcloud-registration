...
def execute_fast_global_registration(source, target, source_fpfh, target_fpfh,
                                     tuple_scale = 0.55, distance_threshold=0.5):
        
    option = o3d.pipelines.registration.FastGlobalRegistrationOption(division_factor = 1.4, tuple_scale=tuple_scale, #0.55
        maximum_correspondence_distance=distance_threshold, iteration_number=1024, decrease_mu = True)
...
def execute_generalized_icp_registration(source, target, max_iterations = 100, distance_threshold = 0.8):
    ...
    result = o3d.pipelines.registration.registration_generalized_icp(
        ...
        icp_type, o3d.pipelines.registration.ICPConvergenceCriteria(max_iteration=max_iterations))
    
    return result

'RUN MULTISTART REGISTRATION'
def execute_multistart_registration(source, target, source_fpfh, target_fpfh, n_attempts = 1, 
                                         tuple_scale=0.9, distance_threshold = 0.8, reg_type = 'fgr'):
...
def refine_registration(source, target, initial_transform,
                        max_iteration = 100, distance_threshold=0.1, icp_type='PointToPoint'):
