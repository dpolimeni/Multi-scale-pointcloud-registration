...
if __name__ == '__main__':
    ...
    target_array, R, t, initial_distortion = perturb_cloud(target_array, deg=deg, mu=mu, std=std,
                                                           n_points=n_points)
    ...
    voxel_size_source = get_voxel_size(pcd_source_ds, target_points = target_points_source, vs_0 = starting_voxel_size, delta = delta, eps = stopping_delta)
    voxel_size_target = get_voxel_size(pcd_target_ds, target_points = target_points_target, vs_0 = starting_voxel_size, delta = delta, eps = stopping_delta)
    print('done')
    ...
    pcd_source_ds, pcd_source_ds_fpfh = preprocess_point_cloud(pcd_source_ds, radius_normal = radius_normal, radius_feature = radius_feature, voxel_size = voxel_size_source)
    pcd_target_ds, pcd_target_ds_fpfh = preprocess_point_cloud(pcd_target_ds, radius_normal = radius_normal, radius_feature = radius_feature, voxel_size = voxel_size_target)
    ...
    start = time.time()
    T_star, coeff_star, rmse_star, errors = compass_search(pcd_source_ds, pcd_target_ds, pcd_source_ds_fpfh, pcd_target_ds_fpfh,  n_attempts = n_attempts,
                                                   delta = compass_delta, eps = compass_eps, max_iter = compass_max_iter, reg_type = reg_type)
    ...
    T_refined = refine_registration(pcd_source_ds, pcd_target_ds,  max_iteration = refinement_iterations,
                                    initial_transform = T_star, distance_threshold=refinement_distnace_threshold,
                                    icp_type=icp_type)
    print('done')
    ...
    path = r'C:/Users/dpolimeni/Desktop/paper_project/{}_{}'.format(reg_type, datetime.strftime(datetime.now(), "%Y-%m-%d-%H-%M"))
